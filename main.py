...
async def ApiList():
    ...
    for a in connection.db.List.find():
        id = '{0}'.format(a['_id'])
        dict = {'name_eng': a.get('name_eng'), 'name_th': a.get('name_th'), 'api_url': a.get(
            'api_url'), 'params': a.get('param1'), 'time': a.get('time')}
        jsonout[id] = dict
    return jsonout
...
async def Logs():
    ...
    for data in connection.db.Logs.find():
        ...
        dict = {'Operation': data.get('Operation'), 'name_eng': data.get('name_eng'), 'name_th': data.get(
            'name_th'), 'api_url': data.get('api_url'), 'params': data.get('param1'), 'time': data.get('time')}
